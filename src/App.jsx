        ...
        {todos.map(text => {
          return (
            <div key={text.id} className="task flex bg-primary w-full rounded-2xl items-center justify-between px-4 my-3">
              <input type="checkbox" id="" className="checkbox mr-auto m-2 bg-orange-500 rounded-2xl " checked={text.Done} name={text.id} onChange={Check} />
              ...
              <div className="right flex space-x-4 ml-auto">
                <div className="edit m-1 cursor-pointer" onClick={(e) => { Edit(e, text.id) }}>Edit</div>
                <div className="delete m-1 cursor-pointer" onClick={(e) => { Delete(e, text.id) }}>Delete</div>
              </div>
            </div>
          )
